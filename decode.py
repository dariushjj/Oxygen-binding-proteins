...
def train(X_train,X_extra,y_train,X_test,y_test,pssm_ssacc_test):
    ...
    x1 = Conv1D(filters={{choice([16 ,  32, 64, 128])}}, kernel_size={{choice([3,5,7, 11, 15, 19, 21])}},
    ...
    x1 = MaxPooling1D(pool_size={{choice([8,16, 32])}}, strides={{choice([8,16, 32, 64])}},name='maxpooling')(x1)
    x1 = BatchNormalization()(x1)

    x1_left = GRU(units={{choice([100,200, 300, 400])}}, recurrent_activation='hard_sigmoid',
             use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
             activation='tanh', return_sequences=True, dropout={{uniform(0, 1)}},name='gru1')(x1)
    x1_right = GRU(units={{choice([100,200, 300, 400])}}, recurrent_activation='hard_sigmoid',
             use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
             activation='tanh', return_sequences=True, dropout={{uniform(0, 1)}},name='gru2',
             go_backwards='True')(x1)
    xgru1 = keras.layers.concatenate([x1_left, x1_right], axis=-1)

    x2_left=GRU(units={{choice([100,200, 300, 400])}}, recurrent_activation='hard_sigmoid',
             use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
             activation='tanh', return_sequences=True, dropout={{uniform(0, 1)}},name='gru3')(xgru1)
    x2_right=GRU(units={{choice([100,200, 300, 400])}}, recurrent_activation='hard_sigmoid',
             use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
             activation='tanh', return_sequences=True, dropout={{uniform(0, 1)}},name='gru4',
    ...
    optim = AdaBound(lr=1e-03, final_lr=0.1, gamma=1e-03, weight_decay=0, amsbound=False)
    ...
    hist = gpu_model.fit(x={'input_embed': X_train, 'input_extra': X_extra},
                         y=y_train,
                         epochs={{choice([50,100,150,200])}}, batch_size={{choice([32,64])}},
...
if __name__ == "__main__":
    # set up logging
    results_dir = "/home/jiajunh/kfold_5"
    '''
    ...
    file = open("/home/jiajunh/kfold_5/" +  "console.txt", 'w')
    sys.stdout = file
    ...
    print(best_run)
    json.dump(best_run, open("/home/jiajunh/kfold_5/model_hyper.txt", 'w'))
