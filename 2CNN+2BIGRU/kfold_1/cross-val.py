...
def prepare_data():
    ...
    train_file = '/home/jiajunh/2CNN+2BIGRU/kfold_1/all_train.pkl'
    pssm_file = '/home/wanglei/data/Oxygen/npy'
    test_file = '/home/jiajunh/2CNN+2BIGRU/kfold_1/all_test.pkl'
...
def save_hist(hist,function):
    model_snapshot_directory = "/home/jiajunh/2CNN+2BIGRU/kfold_1"
    h1 = hist.history
    epoch = hist.epoch
...
def create_model_2CNN():
    ...
    x1 = Conv1D(filters=64, kernel_size=3,activation='relu', strides=1, name='conv1')(x1)
    x1 = Dropout(0.20308, name='drop1')(x1)
    x1 = Conv1D(filters=128, kernel_size=7,
                activation='relu', strides=1, name='conv2')(x1)
    x1 = Dropout(0.00652, name='drop2')(x1)
    x1 = MaxPooling1D(pool_size=8, strides=32, name='maxpooling')(x1)
    
    x1 = Flatten()(x1)
    x1 = Dense(units=256, activation='relu',kernel_regularizer=regularizers.l1_l2(0.0001), name='dense1')(x1)
...
def create_model_2CNN_2BiGRU():
    ...
    embedded = Embedding(max_features, 128, input_length=800)(input_embed)
    x1 = concatenate([embedded, input_extra], axis=2)
    x1 = Conv1D(filters=64, kernel_size=11,
                activation='relu', strides=1,name='conv1')(x1)
    x1 = BatchNormalization()(x1)
    x1 = Activation(activation='relu')(x1)
    x1 = Dropout(0.02007,name='drop1')(x1)
    
    x1 = Conv1D(filters=128, kernel_size=21,
                activation='relu', strides=1,name='conv2')(x1)
    x1 = BatchNormalization()(x1)
    x1 = Activation(activation='relu')(x1)
    x1 = Dropout(0.01850, name='drop2')(x1)
    x_cnn = MaxPooling1D(pool_size=16, strides=32,name='maxpooling')(x1)

    x1_left = GRU(units=200, recurrent_activation='hard_sigmoid',
                  use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
                  activation='tanh', return_sequences=True, dropout=0.22438, name='gru1')(x_cnn)
    x1_right = GRU(units=300, recurrent_activation='hard_sigmoid',
                   use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
                   activation='tanh', return_sequences=True, dropout=0.54717, name='gru2',
                   go_backwards='True')(x_cnn)
    xgru1 = keras.layers.concatenate([x1_left, x1_right], axis=-1)

    x2_left = GRU(units=200, recurrent_activation='hard_sigmoid',
                  use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
                  activation='tanh', return_sequences=True, dropout=0.64088, name='gru3')(xgru1)
    x2_right = GRU(units=200, recurrent_activation='hard_sigmoid',
                   use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
                   activation='tanh', return_sequences=True, dropout=0.00015, name='gru4',
                   go_backwards='True')(xgru1)
    x_gru = keras.layers.concatenate([x2_left, x2_right], axis=-1)
    x = concatenate([x_cnn, x_gru], axis=-1)        

    x = TimeDistributed(Dense(units=256, kernel_regularizer=regularizers.l1_l2(0.0001), name='dense1'))(x)
...
if __name__=='__main__':
    ...
    # model = create_model_2CNN()
    model=create_model_2CNN_2BiGRU()
    ...
    best_model_path = "/home/jiajunh/2CNN+2BIGRU/kfold_1/best_cross_model.h5"
