...
def prepare_data():
    ...
    train_file = '/home/jiajunh/3CNN+protr/kfold_1/all_train.pkl'
    #pssm_file = '/home/wanglei/data/Oxygen/npy'
    test_file = '/home/jiajunh/3CNN+protr/kfold_1/all_test.pkl'
    ...
    X_train_temp = X_all[randomize]
    X_train=reshape(X_train_temp)
    ...
    X_temp=test_df['pdb_feature'].values
    y_temp = test_df['labels'].values
    X_test_temp=np.array(X_temp)
    y_test_temp=np.array(y_temp)
    X_test=reshape(X_test_temp)
...
def train(X_train,X_extra,y_train,X_test,y_test,pssm_ssacc_test):
    ...
    model_snapshot_directory = "home/jiajunh/3CNN+protr/kfold_1"
    max_features = 20
    ...
    input_embed = Input(shape=(20,), name='input_embed')

    x1 = Conv1D(filters={{choice([32,64, 128])}}, kernel_size={{choice([3,5,7, 11, 15, 19, 21])}},
                activation='relu', strides=1,name='conv1')(input_embed)
...
if __name__ == "__main__":
    # set up logging
    results_dir = "/home/jiajunh/CNN/kfold_1"
    '''
    ...
    file = open("home/jiajunh/3CNN+protr/kfold_1/" +  "console.txt", 'w')
    sys.stdout = file
    ...
    print(best_run)
    json.dump(best_run, open("home/jiajunh/3CNN+protr/kfold_1/model_hyper.txt", 'w'))
